// Repetição espaçada (SM-2 simplificado) sobre os SEUS erros, persistida em
// localStorage. Porta de chess-coach/chess_coach/srs.py adaptada ao navegador.

export type Theme = "tatica" | "final" | "meio-jogo" | "abertura";

export interface Card {
  fen: string; // posição antes do lance certo (chave)
  bestSan: string; // o lance a lembrar
  theme: Theme;
  ease: number;
  intervalDays: number;
  reps: number;
  due: number; // epoch ms
  created: number;
  lastNote: string;
}

const KEY = "chess-coach:srs:v1";
const DAY_MS = 86_400_000;

function load(): Record<string, Card> {
  if (typeof window === "undefined") return {};
  try {
    const raw = window.localStorage.getItem(KEY);
    return raw ? (JSON.parse(raw) as Record<string, Card>) : {};
  } catch {
    return {};
  }
}

function persist(cards: Record<string, Card>): void {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(KEY, JSON.stringify(cards));
}

export function addMistake(
  fen: string,
  bestSan: string,
  theme: Theme,
  note = "",
): void {
  const cards = load();
  const existing = cards[fen];
  if (!existing) {
    cards[fen] = {
      fen,
      bestSan,
      theme,
      ease: 2.5,
      intervalDays: 0,
      reps: 0,
      due: Date.now(),
      created: Date.now(),
      lastNote: note,
    };
  } else {
    // Errou de novo antes de dominar: reinicia o ciclo.
    existing.reps = 0;
    existing.intervalDays = 0;
    existing.ease = Math.max(1.3, existing.ease - 0.2);
    existing.due = Date.now();
    if (note) existing.lastNote = note;
  }
  persist(cards);
}

export function dueCards(now = Date.now()): Card[] {
  return Object.values(load())
    .filter((c) => c.due <= now)
    .sort((a, b) => a.due - b.due);
}

export function grade(fen: string, correct: boolean, now = Date.now()): void {
  const cards = load();
  const card = cards[fen];
  if (!card) return;
  if (!correct) {
    card.reps = 0;
    card.intervalDays = 0;
    card.ease = Math.max(1.3, card.ease - 0.2);
    card.due = now + 600_000; // revê em ~10 min, ainda nesta sessão
  } else {
    card.reps += 1;
    if (card.reps === 1) card.intervalDays = 1;
    else if (card.reps === 2) card.intervalDays = 3;
    else card.intervalDays = Math.round(card.intervalDays * card.ease * 100) / 100;
    card.ease = Math.min(3.0, card.ease + 0.05);
    card.due = now + card.intervalDays * DAY_MS;
  }
  persist(cards);
}

export function themeCounts(): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const c of Object.values(load())) {
    counts[c.theme] = (counts[c.theme] ?? 0) + 1;
  }
  return counts;
}

export function totalCards(): number {
  return Object.keys(load()).length;
}
